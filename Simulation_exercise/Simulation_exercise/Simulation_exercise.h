...
private:
	...
	double get_uniform()
	{
		...
		double rand_number = rand() % 1000 + 1;
		return rand_number / 1000;
	}
	...
	double theoretical_value(double q, double p, int n)
	{
		...
		for (int r = 0; r <= n; r++){
			double sum2 = 0.0;
			for (int s = r + 1; s <= n; s++){
				sum2 += take(n, s)*pow(q, s)*pow(1 - q, n - s);
			}
			sum += take(n, r)*pow(p, r)*pow(1 - p, n - r)*sum2;
		}
		return sum;
	}
...
public:
	...
	int search_result()
	{
		...
		for (int n = 2; n <= 100; n++){
			cout << n << endl;
			if (theoretical_value(alice_probability, bob_probability, n + 1) < theoretical_value(alice_probability, bob_probability, n) && theoretical_value(alice_probability, bob_probability, n) >= theoretical_value(alice_probability, bob_probability, n - 1)){
				return n;
			}
			if (theoretical_value(alice_probability, bob_probability, n + 1) <= theoretical_value(alice_probability, bob_probability, n) && theoretical_value(alice_probability, bob_probability, n) > theoretical_value(alice_probability, bob_probability, n - 1)){
				return n;
